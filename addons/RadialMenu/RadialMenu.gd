...
## Number of arcs (menu items) to display.
## Changing this will automatically resize the internal menu_items list.
@export var arc_count: int = 7: set = _set_arc_count
...
var menu_items = []  # will be initialized by _set_arc_count
...
func _set_arc_count(new_count: int):
	if new_count < 0:
		new_count = 0
	arc_count = new_count

	# Adjust menu_items to match arc_count
	while menu_items.size() < arc_count:
		menu_items.append({
			'texture': _get_texture("DefaultPlaceholder"),
			'title': "Item %d" % (menu_items.size() + 1),
			'id': "arc_id%d" % (menu_items.size() + 1)
		})
	while menu_items.size() > arc_count:
		menu_items.pop_back()

	# Re-sync geometry and visuals
	_create_item_icons()
	_calc_new_geometry()
	if visible:
		queue_redraw()
...
func _set_circle_coverage(new_coverage: float):	
	item_angle = new_coverage * 2 * PI / max(1, menu_items.size())
	circle_coverage = new_coverage
	_calc_new_geometry()
	queue_redraw()		
		
func _set_center_angle(new_angle: float):
	item_angle = circle_coverage * 2 * PI / max(1, menu_items.size())
...
func _calc_new_geometry():	
	...
	var angle = circle_coverage * 2.0 * PI / n	
...
func _ready():
	...
	is_ready = true
	_create_subtree()
	# Initialize menu_items based on arc_count
	_set_arc_count(arc_count)
	_calc_new_geometry()
...
func set_items(items):
	...
	_clear_items()
	menu_items = items
	# Keep arc_count in sync (optional: you could also leave it unsynced if you want arc_count to cap display)
	arc_count = menu_items.size()
	_create_item_icons()
	if visible:
		queue_redraw()
...
func add_icon_item(texture : Texture2D, title: String, id):
	...
	arc_count = menu_items.size()  # keep in sync
	_create_item_icons()
	if visible:
		queue_redraw()

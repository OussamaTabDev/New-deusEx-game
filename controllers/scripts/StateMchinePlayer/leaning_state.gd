...
# @export var camera_collision: CollisionShape3D
...
# func _ready() -> void:
# 	# Ensure camera collision is disabled initially
...
# 	# Reset lean
# 	lean_offset = 0.0
# 	# player.CAMERA_CONTROLLER.leaning = false
# 	current_lean = LeanDirection.NONE
# 	#camera_collision.disabled = true
# 	_animate_camera_lean(current_lean)
...
# 		#camera_collision.position.x = lean_vector.x
# 		#camera_collision.position.z = lean_vector.z
# 		# Optional: Add slight camera tilt for more immersion
# 		player.CAMERA_CONTROLLER.rotation.z = -lean_offset * 0.1
# 		#camera_collision.rotation.z = -lean_offset * 0.1

# 	# Animate camera lean (if any additional animations needed)
# 	_animate_camera_lean(current_lean)

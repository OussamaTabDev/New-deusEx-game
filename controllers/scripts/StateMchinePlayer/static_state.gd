class_name  StaticState extends State 


func enter() -> void:
	# player.can_wall_run_bool = true
	# player.SPEED = player.WALK_SPEED * 10
	player.CAMERA_CONTROLLER.static_camera_mode = true
	

func exit() -> void:
	# player.can_wall_run_bool = true
	# player.SPEED = player.WALK_SPEED * 10
	player.CAMERA_CONTROLLER.static_camera_mode = false
